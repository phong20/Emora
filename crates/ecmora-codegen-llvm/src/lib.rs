...
fn build_module<'ctx>(context: &'ctx LlvmContext, program: &Program) -> Result<Module<'ctx>> {
    ...
    let promise_pending = module.add_function(
        "ecmora_promise_pending",
            ...
            match instruction {
                ...
                Instruction::Capture {
                    ...
                    let closure = main
                        ...
                        .into_pointer_value();
                    let dynamic_ptr = builder.build_alloca(dynamic_type, "closure.capture.value")?;
                    builder.build_call(
                        closure_capture,
                        &[
                            closure.into(),
                ...
                Instruction::CellNew {
                    ...
                    let value = values.get(value).copied().context("thiếu cell initial value")?;
                    ...
                    let call = builder.build_call(cell_new, &[dynamic_ptr.into()], "cell.new")?;
                    values.insert(
                        *result,
                ...
                Instruction::CellGet {
                    ...
                    builder.build_call(cell_get, &[cell.into(), dynamic_ptr.into()], "cell.get")?;
                    let dynamic = builder.build_load(dynamic_type, dynamic_ptr, "cell.value.load")?.into_struct_value();
                    values.insert(
                        *result,
                        from_dynamic(
                            &builder,
                ...
                Instruction::CellSet {
                    ...
                    builder.build_store(dynamic_ptr, dynamic)?;
                    builder.build_call(cell_set, &[cell.into(), dynamic_ptr.into()], "cell.set")?;
                }
                ...
                Instruction::ConstString { result, value } => {
                    let global =
                        builder.build_global_string_ptr(value, &format!(".str.{}", result.0))?;
                ...
                Instruction::ToBoolean {
                    ...
                    let value = values
                        .get(operand)
                        .copied()
                        .with_context(|| format!("SSA value %v{} chưa được codegen", operand.0))?;
                ...
                Instruction::UnaryNumber {
                    ...
                    let value = match operator {
                        ...
                        UnaryNumberOperator::BitwiseNot => {
                            let integer = builder.build_float_to_signed_int(
                                value,
                                i32_type,
                                "to.i32",
                            )?;
                            let inverted = builder.build_not(integer, "bitwise.not")?;
                            builder.build_signed_int_to_float(inverted, f64_type, "to.number")?
                        }
                    };
                    values.insert(*result, value.into());
                }
                ...
                Instruction::BinaryNumber {
                    ...
                    let value = match operator {
                        BinaryNumberOperator::Add => builder.build_float_add(left, right, "add")?,
                        ...
                        | BinaryNumberOperator::BitwiseAnd => {
                            let left = builder.build_float_to_signed_int(
                                left,
                                i32_type,
                                "left.i32",
                            )?;
                            ...
                            if *operator == BinaryNumberOperator::ShiftRightZeroFill {
                                builder.build_unsigned_int_to_float(integer, f64_type, "to.number")?
                            } else {
                                builder.build_signed_int_to_float(integer, f64_type, "to.number")?
                            }
                        }
                    };
                    values.insert(*result, value.into());
                ...
                Instruction::CompareNumber {
                    ...
                    let predicate = match operator {
                        ...
                        CompareNumberOperator::NotEqual | CompareNumberOperator::StrictNotEqual => {
                            FloatPredicate::UNE
                        }
                ...
                Instruction::CompareObject {
                    ...
                    values.insert(
                        *result,
                        builder.build_int_compare(predicate, left, right, "object.eq")?.into(),
                    );
                }
                Instruction::Phi { .. } => {}
                ...
                Instruction::CallDirect {
                    ...
                    let dynamic_ptr = builder.build_alloca(dynamic_type, "call.direct.result")?;
                    builder.build_call(
                        target,
                        &[
                ...
                Instruction::CallIndirect {
                    ...
                    let dynamic_ptr = builder.build_alloca(dynamic_type, "call.indirect.result")?;
                    builder.build_call(
                        closure_call,
                        &[
                            closure.into(),
                ...
                Instruction::CallBuiltin {
                    ...
                    for (index, display_value) in display_values.iter().enumerate() {
                        ...
                        if let Some(display_value) = display_value {
                            ...
                            let format = builder.build_global_string_ptr("%s", ".fmt.display")?;
                            builder.build_call(
                                printf,
                                &[
                                ...
                                .context("thiếu kiểu console argument")?
                            {
                                ValueType::Number => {
                                    let format =
                                        builder.build_global_string_ptr("%.15g", ".fmt.number")?;
                                    builder.build_call(
                                        printf,
                                        &[
                                            format.as_pointer_value().into(),
                                ...
                                ValueType::Bool => {
                                    let yes =
                                        builder.build_global_string_ptr("true", ".bool.true")?;
                                    let no =
                                        builder.build_global_string_ptr("false", ".bool.false")?;
